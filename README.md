...
* [`src/buzzlabs_phase_1/`](src/buzzlabs_phase_1/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/buzzlabs_phase_1/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `buzzlabs-phase-1` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/buzzlabs_phase_1/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns buzzlabs-phase-1.example
  (:require [buzzlabs-phase-1.config :as config])
